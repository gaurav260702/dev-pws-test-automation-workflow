...
import influxdb_client.write.Point
    ...
    script {
      ...
      dir('/tmp/reports') {
        ...
        files.each {
          ...
          if(isMasterBranch) {
          def url = 'https://calvinklein-7de56744.influxcloud.net:8086'
          def username = 'INFLUX_DB_USERNAME'
          def password = 'INFLUX_DB_PASSWORD'
          def database = 'k6'

         def influxDBClient = InfluxDBClientFactory.create(url, username, password.toCharArray(), database)
         def writeApi = influxDBClient.getWriteApi()
