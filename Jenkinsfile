    ...
    dir('/tmp/reports') {
      ...
      files.each { f -> 
         ...
         //  echo "${valiDatorJson}"
          if(isMasterBranch) {
          sh """
            curl -i -XPOST "https://calvinklein-7de56744.influxcloud.net:8086/write?db=k6&u=$INFLUX_DB_USERNAME&p=$INFLUX_DB_PASSWORD" --data-binary 'automation_test_report,TEST_NAME=${TEST_NAME},ENV_NAME=${ENV_NAME},TEST_STATUS=${TEST_STATUS},BUILD=${env.GIT_BRANCH}-${env.BUILD_NUMBER} value=1,${statusName}=1,API_RESPONSE="'''${JsonOutput.toJson(configJson.apiCalls)}'''",RESTAPI_CALLS="'''${JsonOutput.toJson(configJson.responseChain)}'''"'
          """
          } 
          else {
